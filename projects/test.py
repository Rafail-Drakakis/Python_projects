import collatz_fibonacci

collatz_fibonacci.collatz_sequence(5)
collatz_fibonacci.fibonacci_sequence(1, 5)

import mp3_downloader

mp3_downloader.download_video_as_mp3("https://www.youtube.com/watch?v=gm-RCYz3f4M")
mp3_downloader.download_playlist_as_mp3("playlist.txt")

import converter

converter.convert_audio("audio.mp3", "audio.wav")
converter.pdf_to_word("sample.pdf")

import count_text

count_text.count_lines("test.txt")
count_text.count_words("test.txt")

import get_fact

get_fact.get_fact(5)

import os

os.remove("sample.docx")
os.remove("Hayko - Yerevani Gishernerum.mp3")
os.remove("Песня День Победы - Лев Лещенко [9 мая] HD.mp3")
os.remove("audio.wav")
