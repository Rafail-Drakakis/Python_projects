...
def download_video(link_entry, is_one):
    ...
    using the `yt-dlp` library. If the `is_one` parameter is set to 1, a message box is
...
def download_from_file(download_func, link_entry, is_one):
    """
...
def graphics():
    ...
    audio_file_button = tk.Button(app, text="Download Audio from File", command=lambda: download_from_file(download_audio, link_entry, 0))
    audio_file_button.pack(padx=10, pady=10)

    # Video download from file button
    video_file_button = tk.Button(app, text="Download Video from File", command=lambda: download_from_file(download_video, link_entry, 0))
